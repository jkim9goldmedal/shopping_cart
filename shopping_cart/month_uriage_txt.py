def month_uriage_txt():
    ...
    import pymysql
    import datetime
    today1 = datetime.datetime.now().strftime("%Y%m")
    filename = 'C:\\Users\\User\\Documents\\shopping_cart\\month_uriage\\Earnings_{}.txt'.format(today1)
    ...
    b = "select sum(税込合計金額),sum(税抜合計金額),sum(税込送料込合計額),sum(税込配送料),sum(税抜配送料),削除フラグ,受注日,sum(税抜合計金額) + sum(税抜配送料)\
        from 受注\
        where DATE_FORMAT(受注日, '%Y/%m') = DATE_FORMAT(now(),'%Y/%m')"
    cur.execute(b)
    goukei = cur.fetchall()
    conn.commit()
    cur.close()
