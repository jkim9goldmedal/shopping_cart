def cart_syori(x):
    ...
    conn.commit()
    uriage = "insert into 受注(顧客ID,受注日,税込合計金額,税抜合計金額,税込送料込合計額,注文合計数,税込配送料,税抜配送料) values(%s,cast(now() as date),%s,%s,%s,%s,%s,%s)"
    ...
    cur.execute("USE shopping_cart")
    conn.commit()
    cartmeisai = "select 商品ID,税込価格*数量,税抜価格*数量,税抜価格,数量,税込価格\
                from カート\
                where 顧客ID = %s"
    ...
    for k in rows1:
        ...
        cur.execute("USE shopping_cart")
        conn.commit()
        meisai = "insert into 受注明細(受注ID,数量,税込金額,税抜金額,商品ID,販売単価,税込販売単価) values(%s,%s,%s,%s,%s,%s,%s)"
