def touroku():
    ...
    while True:
        address = input('住所（30文字以内）>')
        checkaddress = len(address)
        if "'" in address or '"' in address:
            print('入力内容が間違っています。もう一度入力してください。')
        elif 1 <= checkaddress <= 30:
    ...
    while True:
        mail = input('メールアドレス（半角英数字50文字以内）>')
        checkmail = len(mail)
        if "'" in address or '"' in address:
            print('入力内容が間違っています。もう一度入力してください。')
        elif 1 <= checkmail <= 50:
    ...
    while True:
        ...
        if password.encode('utf-8').isalnum() and 5 <= checkpassword <= 10:
            ...
            print('入力文字数が間違っています。もう一度入力してください。')
    return resultname,resultkana,resultzipcode,resultaddress,resulttel,resulthaisouryou,resultmail,resultpassword
