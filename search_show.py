def search_show():
    ...
    #１０：均一税込み　１１：均一価格　１２：在庫数　１３：割引フラグ　14:削除フラグ　１５：商品カテゴリーID
            ...
            cur.execute("USE shopping_cart")
            conn.commit()
            b = "select 在庫.商品ID,商品番号,商品名,商品詳細,TRUNCATE(販売単価*(1+(税率/100)),0),販売単価,TRUNCATE(販売単価*(1-(商品割引率/100))*(1+(税率/100)),0),TRUNCATE(販売単価*(1-(商品割引率/100)),0),TRUNCATE(販売単価*(1-(カテゴリー割引率/100))*(1+(税率/100)),0),TRUNCATE(販売単価*(1-(カテゴリー割引率/100)),0),TRUNCATE(均一単価*(1+(税率/100)),0),均一単価,sum(在庫変動数),割引フラグ,商品.削除フラグ,商品カテゴリーID\
            ...
            for k in rows2:
                ...
                if item_list[13] == '3':
                    ...
                    print(item_list[2],end = '/')
                    print(item_list[3],end = '/')
                    print('{:,}'.format(item_list[4]) + '円',end = '/')
            ...
            cur.execute("USE shopping_cart")
            conn.commit()
            c = "select 在庫.商品ID,商品番号,商品名,商品詳細,TRUNCATE(販売単価*(1+(税率/100)),0),販売単価,TRUNCATE(販売単価*(1-(商品割引率/100))*(1+(税率/100)),0),TRUNCATE(販売単価*(1-(商品割引率/100)),0),TRUNCATE(販売単価*(1-(カテゴリー割引率/100))*(1+(税率/100)),0),TRUNCATE(販売単価*(1-(カテゴリー割引率/100)),0),TRUNCATE(均一単価*(1+(税率/100)),0),均一単価,sum(在庫変動数),割引フラグ,商品.削除フラグ,商品カテゴリーID\
                from 在庫 inner join 商品 on 商品ID = 商品.id inner join 商品カテゴリー on 商品カテゴリーID = 商品カテゴリー.id inner join 税率 on 税率ID = 税率.id\
                group by 在庫.商品ID\
